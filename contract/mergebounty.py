...
class MergeBounty(gl.Contract):
    ...
    def _run_consensus_review(
        ...
        def fetch_evidence(url: str, expected_hash: str) -> typing.Any:
            # Use the SDK's stable convenience GET path and depend only on body bytes.
            # Some Studionet SDK/runtime combinations do not expose HTTP status fields
            # on Response, so status/status_code must not be accessed here. A 404/error
            # body naturally fails the committed SHA-256 check and becomes insufficient
            # evidence instead of crashing consensus.
            try:
...
"""
        ...
        def validator_fn(leader_result) -> bool:
            ...
            try:
                ...
                # Scores and prose are intentionally NOT consensus-critical.
                # Different competent validators may assign different numeric scores
                # while reaching the same settlement decision. The verdict already
                # incorporates the deterministic score/quality guards in leader_fn.
                return True
            except Exception:
                return False
    ...
    @gl.public.write
    def probe_evidence(self, evidence_url: str, evidence_hash: str) -> typing.Any:
        """
        Zero-value preflight for the exact commit-bound evidence path.
        This intentionally does not change bounty state. It lets us verify that
        Studionet validators can fetch the URL and reproduce the expected SHA-256
        before any new escrow is funded.
        """
        evidence_url = evidence_url.strip()
        evidence_hash = evidence_hash.strip().lower()
        if not evidence_url.startswith("https://"):
            raise gl.vm.UserError("evidence_url must use https://")
        self._require_sha256(evidence_hash, "evidence_hash")

        def fetch_probe() -> dict:
            response = gl.nondet.web.get(evidence_url)
            body = response.body
            if not isinstance(body, (bytes, bytearray)):
                raise gl.vm.UserError("Evidence response body is not bytes")
            raw = bytes(body)
            digest = hashlib.sha256(raw).hexdigest()
            return {
                "digest": digest,
                "hash_match": digest.lower() == evidence_hash,
                "byte_length": len(raw),
            }

        # The exact raw GitHub commit URL is immutable, so these derived fields
        # should be identical across validators.
        return gl.eq_principle.strict_eq(fetch_probe)
